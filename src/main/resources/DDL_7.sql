...
-- 검사결과정의
DROP TABLE IF EXISTS RES_DEFINE RESTRICT;

-- 설계도 파일
DROP TABLE IF EXISTS PLAN_FILE RESTRICT;
...
-- 검사결과정의
CREATE TABLE RES_DEFINE (
  define VARCHAR(100) NOT NULL COMMENT '결과정의' -- 결과정의
)
COMMENT '검사결과정의';

-- 검사결과정의
ALTER TABLE RES_DEFINE
  ADD CONSTRAINT PK_RES_DEFINE -- 검사결과정의 기본키
    PRIMARY KEY (
      define -- 결과정의
    );
...
CREATE TABLE PLAN_FILE (
  rno      INTEGER         NOT NULL COMMENT '파일일련번호', -- 파일일련번호
  pno      INTEGER         NOT NULL COMMENT '설계도일련번호', -- 설계도일련번호
  keyvalue VARCHAR(45)     NULL     COMMENT '파일키', -- 파일키
  pic      VARBINARY(1000) NULL     COMMENT '이미지', -- 이미지
  video    VARBINARY(1000) NULL     COMMENT '영상', -- 영상
  link     VARCHAR(255)    NULL     COMMENT '링크', -- 링크
  chk      VARCHAR(3)      NULL     COMMENT '표시여부', -- 표시여부
  pnocopy  INTEGER         NULL     COMMENT '임시일련번호' -- 임시일련번호
)
COMMENT '설계도 파일';

-- 설계도 파일
ALTER TABLE PLAN_FILE
  ADD CONSTRAINT PK_PLAN_FILE -- 설계도 파일 기본키
    PRIMARY KEY (
      rno, -- 파일일련번호
      pno  -- 설계도일련번호
    );
...
CREATE TABLE PLAN (
  cono  INTEGER         NOT NULL COMMENT '설계도일련번호', -- 설계도일련번호
  eno   INTEGER         NOT NULL COMMENT '멘토일련번호', -- 멘토일련번호
  plmap VARCHAR(20000)  NOT NULL COMMENT '설계도', -- 설계도
...
CREATE TABLE RESULT (
  ...
  eare VARCHAR(100) NOT NULL COMMENT '개별결과' -- 개별결과
)
COMMENT '적성검사결과';
...
-- 설계도 파일
ALTER TABLE PLAN_FILE
  ADD CONSTRAINT FK_PLAN_TO_PLAN_FILE -- 설계도 -> 설계도 파일
    FOREIGN KEY (
      pno -- 설계도일련번호
    )
    REFERENCES  PLAN ( -- 설계도
...
-- 적성검사결과
ALTER TABLE RESULT
  ADD CONSTRAINT FK_RES_DEFINE_TO_RESULT -- 검사결과정의 -> 적성검사결과
    FOREIGN KEY (
      rer -- 검사결과
    )
    REFERENCES RES_DEFINE ( -- 검사결과정의
      define -- 결과정의
    );
...
-- 추천주제
ALTER TABLE SNATR
  ADD CONSTRAINT FK_RES_DEFINE_TO_SNATR -- 검사결과정의 -> 추천주제
    FOREIGN KEY (
      define -- 결과정의
    )
    REFERENCES  RES_DEFINE ( -- 검사결과정의
